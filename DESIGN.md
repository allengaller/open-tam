...
**The No-Mid-Word-Break Rule.** Headings use `text-wrap: balance` so CJK terms are never stranded mid-word ("显性化" must not split across lines). The hero `.sub` uses `word-break: keep-all` so line breaks only land on the `·` separators, never inside a word.
